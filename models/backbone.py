...
# from models.misc import clean_state_dict
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        if name in ['resnet18', 'resnet50', 'resnet34', 'resnet101']:
            print(f"Backbone kinds:{name}")
            backbone = getattr(torchvision.models, name)(
                replace_stride_with_dilation=[False, False, dilation],
                pretrained=False)
            # print("\nParams from plain backbone need to update:")
            # for name, param in backbone.named_parameters():
            #     if param.requires_grad == True:
            #         print("\t", name)
            # print("\n\n")
            print(f"return interm layers:{return_interm_layers}")
            if return_interm_layers:
                return_layers = {"layer1": "0", "layer2": "1", "layer3": "2", "layer4": "3"}
            else:
                return_layers = {'layer4': "0"}
            print("\nParams from plain backbone after loading:")
            for layer, param in backbone.named_parameters():
                if param.requires_grad == True:
                    print("\t", layer)
            print("\n\n")
            # exit()
        ...
        print(f"is train_backbone:{train_backbone}")
...
def build_backbone(args):
    ...
    train_backbone = True
    print("In build backbone")
    print(f"backbone use:{args.backbone}")
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        ...
        print("Params from BACKBONE need to update:")
        for name, param in backbone.named_parameters():
            if param.requires_grad == True:
                print("\t", name)
        bb_num_channels = backbone.num_channels
    model = Joiner(backbone, position_embedding, args)
    print("Params from JOINER need to update:")
...
if __name__ == "__main__":
    ...
    print("\nParams from backbone need to update:")
    for name, param in backbone.named_parameters():
        if param.requires_grad == True:
            print("\t", name)